...
static char* sys_getcwd(char *buf, size_t size) {
    int err=0;
    size_t ucs2size = size;
    uint16_t*res = 0, *ucs2buf = getbytes(ucs2size*sizeof(*ucs2buf));

    if(!ucs2buf)
        return 0;
    res = _wgetcwd(ucs2buf, ucs2size);

    if(res) {
        u8_ucs2toutf8(buf, size-1, ucs2buf, -1);
        buf[size-1] = 0;
        sys_unbashfilename(buf, buf);
    } else
        err=1;

    freebytes(ucs2buf, ucs2size*sizeof(*ucs2buf));
    return (err)?0:buf;
}
...
#ifdef _WIN32
            if(x->x_verbose)
                pd_error(x, "the path \"%s\" contains an illegal character '%c'.", path, *s);
            err_count++;
#else
            if(x->x_verbose)
